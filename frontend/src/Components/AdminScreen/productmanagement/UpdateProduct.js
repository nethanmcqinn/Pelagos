...
const UpdateProductContent = React.memo(({
  ...
  const handleDiscountedPriceChange = (text) => {
    ...
    // Calculate percentage when price is manually edited
    if (formData.price && cleaned) {
    ...
    <ScrollView style={styles.container}>
      <View style={styles.form}>
        ...
        >
          ...
          <Icon name="arrow-drop-down" size={24} color="#9ab4d3" />
        </TouchableOpacity>
        ...
        {showDiscountSection && (
          <View style={styles.discountSection}>
            ...
            >
              ...
              <Icon name="calendar-today" size={20} color="#9ab4d3" />
            </TouchableOpacity>
          ...
          <View style={styles.modalContent}>
            ...
            {categories.length === 0 ? (
              <View style={styles.emptySuppliers}>
                <Text style={styles.emptySuppliersText}>No categories available. Add categories first.</Text>
              </View>
            ) : (
              <FlatList
                data={categories}
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    <Text style={[
                      styles.categoryText,
                      formData.category === item.name && styles.categoryTextSelected,
                    ]}>
                      {item.name}
                    </Text>
                  </TouchableOpacity>
...
export default function UpdateProductScreen({ navigation, route }) {
  ...
  const validateForm = () => {
    ...
    // Validate discount fields if discount section is enabled
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Add discount fields if section is enabled
      if (showDiscountSection) {
        ...
        // If discount section is disabled, clear discount fields
      ...
      let errorMessage = 'Failed to update product. Please check all fields.';
